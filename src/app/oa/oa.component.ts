...
enhance(id:string){
  ...
  $('.inputId').prop('readonly', true);
  this.http
  .post('http://127.0.0.1:8000/enhance',{}).subscribe((data)=>(temp_row=data))
  this.sleep(3000).then(() => {
    console.log("dropdown")
    console.log(temp_row)
   this.cars=temp_row['result']
...
automaticfill(){
  ...
  this.sleep(3000).then(() => {
    ...
    for(let index=0;index<temp_row['result'].length;index++){
      for(let inner_index=0;inner_index<this.rows[index].Level_value.length;inner_index++)
      {
        this.rows[index]['Level_value'][inner_index].value=temp_row['result'][index][inner_index]
      }
    }
  })
}
